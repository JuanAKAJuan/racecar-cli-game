...
/**
 *Displays the m_Weights of the current m_Track.  Used for Testing/Debugging Purposes
 */
...
void Racetrack::MoveUserCar()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        if (m_Weights[int(round(m_OldYCoordinate))][int(round(m_OldXCoordinate))] == m_W_FINISH)
        {
            WinningScreen();
            DeleteOldUserCar();
            UpdateUserPosition();
...
void Racetrack::UpdateUserPosition()
{
    ...
    // Reversed the x and y Coordinates since they make more sense for x to be associated with columns
    // and y to be associated with rows when it comes to movement.
...
void Racetrack::CheckUserCollision()
{
    ...
    bool handleCollision = m_Track[m_NewYCoordinate][m_NewXCoordinate] ==
                           m_Track[handle.GetRowNumber()][handle.GetColumnNumber()];
    bool userWallCollision = m_Weights[m_NewYCoordinate][m_NewXCoordinate] == m_W_WALL;

    if (speedCollision || handleCollision)
    {
        // Notifies the user they hit a car, resets their velocities, and lowers their max speed by 1
        std::cout << "You hit a car! Your velocity has been reset and max speed has been decreased." << std::endl;
        m_XCoordinate = 0;
        m_YCoordinate = 0;
    ...
    {
        // Notifies the user they hit a wall, resets their velocities, and lowers their max speed by 1
...
void Racetrack::WinningScreen()
{
    ...
    std::cout << "  |_| \\___/ \\___/     \\_/\\_/  \\___/|_| \\_(_)" << std::endl;
}
