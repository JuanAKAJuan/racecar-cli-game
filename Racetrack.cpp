 ...
 */
char Racetrack::GetTrack(int y, int x) { return m_Track[y][x]; }

/**
 *Update an element from the m_Track
 */
void Racetrack::SetTrack(int y, int x, char value) { m_Track[y][x] = value; }

/**
 *Get value of weight location
 **/
int Racetrack::GetWeight(int y, int x) { return m_Weights[y][x]; }

/**
 *Update a weight from the weight m_Track
 */
void Racetrack::SetWeight(int y, int x, int value) { m_Weights[y][x] = value; }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (checkFinish) {
            ...
            if (checkOldWall || checkNewWall) {
                ...
                if (speedOldCollision || handleOldCollision || speedNewCollision ||
                    handleNewCollision) {
